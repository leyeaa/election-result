...
$security_message=trim(strtoupper($arry_msg2[2])); // what message
	 ...
	 if($len == 4){  
	  ...
	  {
	   //true
			 $msg_in="Duplicate Entry, but last Message is taking: $state_unit_code*APC$APC_score*LP$LP_score*PDP$PDP_score ";
			 ...
			 mysql_query($hh) or die(mysql_error());
			   //--------------------------------
			$status=1;
	        $sqly="INSERT INTO result SET unit_code='$unit_code',unit_name='$unit_name',APC='$APC_score',LP='$LP_score',PDP='$PDP_score',lg_code='$lg_code',ward_code='$ward_code',state_code='$state_code',status='$status',agent_phone='$receiver'";
	   mysql_query($sqly) or die(mysql_error()); 
   ...
   $url="http://www.ermsng.org/resultapi.php?unit_code=$unit_code&unit_name=$unit_name&ward_code=$ward_code&lg_code=$lg_code&state_code=$state_code&PDP=$PDP_score&APC=$APC_score&LP=$LP_score";
	   ...
	   mysql_query($sqlb) or die(mysql_error());
	   //-------------------------------------------------------------
	   $msg_in="Thank you we have recieved your result $state_unit_code*APC$APC_score*LP$LP_score*PDP$PDP_score";
	   $sql="INSERT INTO messageout SET MessageFrom='$sender',MessageTo='$receiver',MessageText='$msg_in'";
	   mysql_query($sql) or die(mysql_error());
		 //Delete from Message In
	   $sqld="DELETE FROM messagein WHERE Id='$id'";
       mysql_query($sqld) or die(mysql_error);
				 ...
				 {
				 $msg_in="Incorrect Format! Resend in this format:- $state_unit_code*APC*LP*PDP";
	  ...
	  {
		$msg_in="Polling Unit cant be found, check the code and resend:- $state_unit_code*APC*LP*PDP";
	             $sqlc="INSERT INTO messageout SET MessageFrom ='$sender',MessageTo ='$receiver',MessageText ='$msg_in'";
	             mysql_query($sqlc) or die(mysql_error());
				 //=========================== insert
				  $sqlb2="INSERT INTO wrong_messagein SET MessageFrom ='$receiver',MessageText ='$msg2'";
