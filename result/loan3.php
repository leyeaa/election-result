...
$blow=explode("*",$msg2);
$msg_inter=$blow[2];
$num_array=count($blow);



   if($msg_inter==1 || 2 || 3 || 4 || 5)
     {	
     $sepoll=$blow[2];  
...
//==============================
   /*if($arry_msg2[2]==2)
     {
	  //echo"adeyemi fgggf";
	  $sql="INSERT INTO security2 (unit_id,security_code,sender) VALUES(?,?,?)";
	  $params = array($unit_id,$arry_msg2[2],$receiver);
	  sqlsrv_query($dbconnect,$sql,$params) or die(sqlsrv_errors());
	  //================================Delete from  ozekimessagein====================================================
	   $sqld="DELETE FROM ozekimessagein WHERE id ='$id'";
       sqlsrv_query($dbconnect,$sqld) or die("can not delete");
     }   */
