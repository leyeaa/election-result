...
$ck="SELECT * FROM result WHERE unit_code='$unit_code'";
	   ...
	   if($ro=mysql_fetch_array($rk)){
		 $gg="UPDATE result SET APC='$APC',AD='$AD',SDP='$SDP',PDP='$PDP',LP='$LP' WHERE unit_code='$unit_code'";
	    mysql_query($gg) or die(mysql_error());	
	    }
	    else{
		 $gg="INSERT INTO result SET unit_name='$unit_name',unit_code='$unit_code',lg_code='$lg_code',ward_code='$ward_code',state_code='$state_code',APC='$APC',AD='$AD',SDP='$SDP',PDP='$PDP',LP='$LP'";
	    mysql_query($gg) or die(mysql_error());	
		}
?>
