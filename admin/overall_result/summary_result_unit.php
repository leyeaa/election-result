...
function state_result($party_name,$ward_code){
	global $database;
$sql2="SELECT SUM($party_name) AS tot FROM result WHERE ward_code='$ward_code'";
	   $result2=$database->query($sql2) or die($database->error());
	   $rows2=$database->fetch_array($result2);
	   $result=$rows2['tot'];
	   return $result;	
...
<?php	
    ...
    while($rra4=$database->fetch_array($resa4)){
        ...
        $party_result=state_result($party_name,$ward_code);
