...
//--------------------------------------------
///registered Voters
function registered_voters($other_name,$lg_code,$state_code){
	global $database;
    $sql2="SELECT SUM($other_name) AS tot FROM polling_unit WHERE lg_code='$lg_code' AND state_code='$state_code'";
	   $result2=$database->query($sql2) or die($database->error());
	   $rows2=$database->fetch_array($result2);
	   $result=$rows2['tot'];
	  return $result;	
